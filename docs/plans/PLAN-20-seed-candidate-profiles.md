...
### 2026-04-19 — Post-seed additions

Follow-ups built on top of the initial roster, each shipped as its own commit:

1. **Round-aware visible candidate cap** — new `src/utils/contestRounds.ts` exporting `getRoundCandidateLimit(roundId)` (returns `undefined` for unlimited, else the per-round visible count). Round 1 is uncapped so the whole roster is visible in Endorsement One; Rounds 2+ narrow the field (20 → 10 → 4 → 2 → 1). Threaded through `getApprovedCandidates`, `getCandidatesForFeed`, and `getCandidatesWithUsers` (native + web variants). All UI surfaces (For You, leaderboard, filter sheet, character search) pass `getRoundCandidateLimit(currentRoundId)`.
2. **Rank-based leaderboard cutoff** — sibling helper `getRoundAdvancementCount(roundId)` (R1→20, R2→10, R3→4, VTH→2, Debate→1; terminal rounds return `undefined`). Replaces the old fixed `endorsementCutoffs[0].threshold` line; cutoff index is now `advancementCount` rather than "first row below 1000 endorsements." Label reads `Top {N} advance`. Terminal rounds render no line.
3. **Real PSA autoplay in For You** — feed loader was building synthetic PSA objects with empty `videoUrl`, so `FullScreenPSA` always fell back to the headshot even for candidates with a real PSA. New helper `getPublishedPSAsByCandidate()` batch-fetches all published PSAs once per feed load and returns a `Map<candidateId, PSA>` (most-recent published per candidate). `generateFeedItem` merges real `videoUrl` / `thumbnailUrl` / `title` / `description` / `duration` / `issueIds` into each matching feed item. Candidates without a PSA still fall back to the headshot.
4. **256 px avatar thumbnails** — extended the seeder with a `ThumbnailPlan` that runs in parallel with `AvatarPlan`. `generateAvatarThumbnail()` shells out to macOS `sips` (`-Z 256 -s format jpeg -s formatOptions 80`) to produce ~20 KB JPEGs from the 1+ MB source PNGs. Uploaded to `gs://party-nomination-app.firebasestorage.app/profileThumbnails/{userId}/thumbnail.jpg`. Candidate + user docs carry `thumbnailUrl` + `thumbnailHash` fields; hash is `{sourceHash}/{THUMBNAIL_VERSION}` (`THUMBNAIL_VERSION = 'v1-256-jpeg-q80'`), so re-runs skip unchanged thumbs but regenerate if params ever change. 202 thumbnails uploaded on this run. `CandidateAvatar` prefers `thumbnailUrl` over `photoUrl`; the For You full-screen background still loads the full photo for quality. Per-avatar bandwidth dropped ~75×.
5. **Thumbnails tracked in git** — `assets/candidates/PA-01-Profile-Thumbs/` (2.0 MB) and `assets/candidates/PA-02-Profile-Thumbs/` (2.1 MB), 101 JPEGs per district. 4.1 MB total vs. 270 MB of untracked source PNGs. Gives anyone cloning the repo a portable reference set without the bulk of the originals.
6. **Full-size photo viewer on candidate profile** — `src/screens/CandidateDetailScreen.tsx`. Tapping the header avatar opens a bottom-sheet modal (slide-up, grab handle, backdrop-tap dismiss) with the full-resolution photo at ~75% height, candidate name as title, and the Bio tab's "About" + "Why I'm Running" sections rendered under the image. Follows the same `QuizBottomSheet` / `VerifyIdentitySheet` pattern (native `Animated` + web `Portal`). Tap is a no-op when no photo exists so DiceBear/initials fallbacks don't open an empty viewer.

Dependency note: the avatar thumbnail step depends on macOS `sips`. Non-macOS contributors running the seeder will hit an error on the sips spawn — swap in a cross-platform tool (e.g., sharp) if the pipeline ever runs off macOS.

### 2026-04-19 — Web-only follow-ups and endorsement count fix

After deploying the seeded roster to the live web build (`party-nomination-app.web.app` + `mainstreet-party.web.app` via Firebase Hosting), a handful of web-specific issues surfaced and were fixed:

1. **PSA tab cards oversized** — `(SCREEN_WIDTH - 44) / 2` captured `Dimensions.get('window').width` at module load, which on web is the browser window (1400+ px), not the phone frame. PSA cards now render as a single full-width column on every platform (web stacks vertically; native had used a two-col grid which was dropped for consistency).
2. **Back nav from candidate profile skipped the tab** — `router.back()` on web was popping through the expo-router tab boundary and landing on Home; re-tapping "For You" returned to the candidate because the tab's stack state still held it. `AppHeader`'s back button now detects the nested candidate route via `usePathname` and calls `router.replace` to the parent tab.
3. **Tab tap didn't reset stack on web** — iOS Tabs auto-pop to the tab root on re-tap of the focused tab; web does not. Added a `tabPress` listener to each main tab in `app/(main)/_layout.tsx` that preventDefaults and `router.replace`s to the tab root on web.
4. **Photo sheet content truncated on web** — `height:'75%'` + `ScrollView flex:1` inside an `Animated.View` doesn't scroll reliably on React Native Web. Switched to the `FilterByAreaSheet` pattern (`maxHeight` + `flexGrow: 0`) and shrunk the image to 260 px on web (vs. 340 on native) so more of the About + Why I'm Running text is visible without scrolling.
5. **Leaderboard load was slow** — `getCandidatesWithUsers` was fetching user docs sequentially via `for (await getUser)`, so Round 1's 202 candidates meant 202 serial roundtrips. Switched to `Promise.all`. Roughly 20–50× faster; Round 1 now loads in 1–3 s.
6. **Endorsement count didn't increment when endorsing from Bookmarks** — `createEndorsement` in the Firestore service had an idempotency check that returned an existing active endorsement's ID without incrementing the count. If local state and Firestore had drifted (e.g., across sessions), the user saw `Endorse All` flip the candidate to endorsed without a `+1`; revoking later decremented the count, leaving it lower than before they ever interacted. Fix: count updates moved from the Firestore service into `userStore.endorseCandidate` / `revokeEndorsement`, guarded by the local `hasEndorsedCandidate` check. See `docs/TROUBLESHOOTING.md` for the full writeup and the architectural rule ("Firestore layer owns endorsement doc state; userStore owns the count").
