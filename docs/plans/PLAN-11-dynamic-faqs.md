...
- Possible stages defined in `src/types/index.ts`: `'pre_nomination' | 'nomination' | 'voting' | 'post_election'` — only 4 stages, too coarse
- `configStore.ts` already subscribes to `partyConfig` in real-time via `onSnapshot`, which is correct

## Files to Modify

- `src/components/home/VoterHome.tsx` — replace hardcoded FAQs with dynamic lookup
- `src/types/index.ts` — add `ContestRoundId` type per PLAN-00
- `src/stores/configStore.ts` — add `currentRound` derived state, `fetchContestRounds()`, and new selectors per PLAN-00
