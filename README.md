...
The 202 fictional candidates per district live in `scripts/data/candidates-PA-{01,02}.json` (source of truth, committed to git). Avatars live locally at `assets/candidates/PA-{01,02}-Profile-Pics/` (uploaded to Firebase Storage during seeding, source PNGs are untracked). PSA videos go at `assets/candidates/PA-{01,02}-PSA-Videos/` (opt-in per candidate).

256 px JPEG thumbnails at `assets/candidates/PA-{01,02}-Profile-Thumbs/` ARE tracked in git (4.1 MB total) — generated by the seeder via `sips` and uploaded to `profileThumbnails/{userId}/thumbnail.jpg`. Avatar-sized surfaces (leaderboard, filter sheet, character search, candidate detail header) load the thumbnail; the For You full-screen background loads the full photo. The avatar on the candidate profile is tappable — opens a bottom-sheet viewer with the full-resolution photo, candidate name, and the Bio tab's About + Why I'm Running text.
...
See **[docs/plans/PLAN-20-seed-candidate-profiles.md](./docs/plans/PLAN-20-seed-candidate-profiles.md)** for the full design, including hash-diff upload behavior, safety gates (typed confirmation, emulator smoke test, backup recipe), PSA auto-thumbnail generation via `ffmpeg`/`ffprobe`, and avatar thumbnail generation via macOS `sips`.
