...
import { firebaseApp, Collections } from './config';
import type {
  User,
  Candidate,
...
export const getCandidate = async (candidateId: string): Promise<Candidate | null> => {
  const docSnap = await getDoc(doc(db, Collections.CANDIDATES, candidateId));
...
export const createCandidateApplication = async (
  ...
  try {
    ...
    const docRef = await addDoc(collection(db, Collections.CANDIDATE_APPLICATIONS), {
      ...data,
...
export const getCandidateApplication = async (
  ...
  const docSnap = await getDoc(doc(db, Collections.CANDIDATE_APPLICATIONS, applicationId));
  return docSnap.exists() ? (docSnap.data() as CandidateApplication) : null;
};
...
export const getUserCandidateApplication = async (
  ...
  const q = query(
    collection(db, Collections.CANDIDATE_APPLICATIONS),
    where('userId', '==', userId),
    orderBy('submittedAt', 'desc'),
    firestoreLimit(1)
  );
  const snapshot = await getDocs(q);
  return snapshot.empty ? null : (snapshot.docs[0].data() as CandidateApplication);
};
...
export const createPSA = async (
  ...
  const now = Timestamp.now();
  const docRef = await addDoc(collection(db, Collections.PSAS), {
    ...data,
    createdAt: now,
    updatedAt: now,
  });
...
export const getPSA = async (psaId: string): Promise<PSA | null> => {
  const docSnap = await getDoc(doc(db, Collections.PSAS, psaId));
  return docSnap.exists() ? (docSnap.data() as PSA) : null;
};
...
export const updatePSA = async (
  ...
  await updateDoc(doc(db, Collections.PSAS, psaId), {
    ...data,
    updatedAt: Timestamp.now(),
  });
};
...
export const getIssuesByCategory = async (category: string): Promise<Issue[]> => {
  ...
  const snapshot = await getDocs(q);
  return snapshot.docs.map((d) => d.data() as Issue);
};
...
export const seedIssues = async (): Promise<void> => {
  ...
  for (const issue of issues) {
    const docRef = doc(db, Collections.ISSUES, issue.id);
...
const generateAllIssuePositions = (
  ...
  customPositions?: Record<string, { position: string; spectrum: number }>
...
export const seedCandidates = async (): Promise<void> => {
  const candidates: Array<{
    ...
    },
    {
      ...
      gender: 'male',
      bio: { summary: 'Former teacher and school board president focused on education equity.', background: 'Taught in public schools for 20 years.', education: [{ institution: 'Boston College', degree: 'Education', year: 1998 }], experience: [{ title: 'School Board President', organization: 'Boston Public Schools', startYear: 2018, description: 'Increased funding equity' }], achievements: ['Reduced achievement gap by 15%', 'Expanded pre-K access'] },
    ...
    },
    {
      ...
      bio: { summary: 'Healthcare administrator working to expand access and lower costs.', background: 'Ran community health centers serving 50,000 patients.', education: [{ institution: 'Johns Hopkins', degree: 'Public Health', year: 2006 }], experience: [{ title: 'CEO', organization: 'Community Health Network', startYear: 2014, description: 'Expanded services to 12 communities' }], achievements: ['Reduced ER visits by 30%', 'Pioneered telehealth programs'] },
    ...
    },
    {
      ...
      bio: { summary: 'Environmental engineer promoting practical clean energy solutions.', background: 'Built renewable energy projects across 15 states.', education: [{ institution: 'MIT', degree: 'Engineering', year: 2004 }], experience: [{ title: 'Founder', organization: 'GreenTech Solutions', startYear: 2010, description: 'Deployed 500MW of solar capacity' }], achievements: ['Created 2,000 clean energy jobs', 'Reduced costs 40%'] },
    ...
    },
    {
      ...
      bio: { summary: 'Former prosecutor focused on smart justice reform and community safety.', background: 'Prosecuted violent crimes while advocating for rehabilitation.', education: [{ institution: 'Georgetown Law', degree: 'JD', year: 2007 }], experience: [{ title: 'Assistant DA', organization: 'District Attorney Office', startYear: 2007, description: 'Led conviction integrity unit' }], achievements: ['Launched diversion programs', 'Reduced recidivism 25%'] },
    ...
    },
    {
      ...
      bio: { summary: 'Business owner and former mayor focused on pragmatic, bipartisan solutions.', background: 'Built successful manufacturing company, served two terms as mayor.', education: [{ institution: 'University of Michigan', degree: 'Business', year: 1995 }], experience: [{ title: 'Mayor', organization: 'City of Grand Rapids', startYear: 2016, description: 'Balanced budget while improving services' }], achievements: ['Attracted $500M in investment', 'Bipartisan infrastructure plan'] },
    ...
    },
    {
      ...
      bio: { summary: 'Retired military officer committed to strong defense and diplomatic engagement.', background: '25 years in the Army.', education: [{ institution: 'West Point', degree: 'Military Science', year: 1992 }], experience: [{ title: 'Colonel', organization: 'US Army', startYear: 1992, endYear: 2017, description: 'Commanded 3,000 troops' }], achievements: ['Bronze Star recipient', 'Led successful humanitarian missions'] },
    ...
    },
    {
      ...
      bio: { summary: 'Tech entrepreneur promoting innovation and economic opportunity.', background: 'Founded two successful startups.', education: [{ institution: 'Stanford', degree: 'Computer Science', year: 2005 }], experience: [{ title: 'CEO', organization: 'TechForward Inc', startYear: 2010, description: 'Built company to $100M valuation' }], achievements: ['Forbes 30 Under 30', 'Founded coding bootcamp for underserved youth'] },
    ...
    },
    {
      ...
      bio: { summary: 'Nonprofit leader bridging divides on housing.', background: 'Led Habitat for Humanity chapter.', education: [{ institution: 'Notre Dame', degree: 'Social Work', year: 2003 }], experience: [{ title: 'Executive Director', organization: 'Habitat for Humanity', startYear: 2010, description: 'Tripled home production' }], achievements: ['National nonprofit leader of the year', 'Bipartisan housing coalition'] },
    ...
    },
    {
      ...
      bio: { summary: 'Farmer and rural advocate.', background: 'Third-generation farmer.', education: [{ institution: 'Iowa State', degree: 'Agriculture', year: 2000 }], experience: [{ title: 'President', organization: 'State Farm Bureau', startYear: 2015, description: 'Advocated for 50,000 farmers' }], achievements: ['Expanded broadband to rural areas', 'Trade deal improvements'] },
    ...
    },
    {
      ...
      bio: { summary: 'Small business owner advocating for lower taxes.', background: 'Built chain of hardware stores.', education: [{ institution: 'Texas A&M', degree: 'Business', year: 1990 }], experience: [{ title: 'Owner', organization: 'Turner Hardware', startYear: 1995, description: 'Grew to 25 locations' }], achievements: ['Small Business of the Year', 'Chamber of Commerce president'] },
    ...
    },
    {
      ...
      bio: { summary: 'Former school principal promoting parental choice.', background: 'Led turnaround of failing schools.', education: [{ institution: 'Vanderbilt', degree: 'Education Leadership', year: 1998 }], experience: [{ title: 'Principal', organization: 'Success Academy', startYear: 2005, description: 'Improved test scores 40%' }], achievements: ['Principal of the Year', 'Launched STEM magnet program'] },
    ...
    },
    {
      ...
      bio: { summary: 'Sheriff focused on law and order.', background: '30 years in law enforcement.', education: [{ institution: 'Sam Houston State', degree: 'Criminal Justice', year: 1990 }], experience: [{ title: 'Sheriff', organization: 'County Sheriff Department', startYear: 2012, description: 'Reduced crime 20%' }], achievements: ['Officer of the Year', 'Implemented community policing'] },
    ...
    },
    {
      ...
      bio: { summary: 'Healthcare executive promoting market-based reforms.', background: 'Led hospital system.', education: [{ institution: 'Duke', degree: 'Healthcare Administration', year: 2002 }], experience: [{ title: 'CEO', organization: 'Regional Medical Center', startYear: 2012, description: 'Improved quality rankings' }], achievements: ['Top 100 hospitals', 'Pioneered price transparency'] },
    ...
    },
    {
      ...
      bio: { summary: 'Energy executive promoting American energy independence.', background: 'Led diversified energy company.', education: [{ institution: 'Texas Tech', degree: 'Petroleum Engineering', year: 1988 }], experience: [{ title: 'CEO', organization: 'Palmer Energy', startYear: 2005, description: 'Managed 5,000 employees' }], achievements: ['Increased domestic production', 'Invested in carbon capture'] },
    ...
    },
    {
      ...
      bio: { summary: 'Constitutional conservative fighting to limit government.', background: 'Constitutional lawyer.', education: [{ institution: 'Yale Law', degree: 'JD', year: 1995 }], experience: [{ title: 'Senior Counsel', organization: 'Liberty Legal Foundation', startYear: 2000, description: 'Defended constitutional rights' }], achievements: ['Won religious liberty cases', 'State legislator'] },
    ...
    },
    {
      ...
      bio: { summary: 'Pro-life advocate and family values champion.', background: 'Founded crisis pregnancy center network.', education: [{ institution: 'Liberty University', degree: 'Public Policy', year: 2000 }], experience: [{ title: 'President', organization: 'Family First Foundation', startYear: 2005, description: 'Built 50 centers nationwide' }], achievements: ['Helped 10,000 women choose life', 'Family policy advisor'] },
    ...
    },
    {
      ...
      bio: { summary: 'Border hawk and immigration enforcement advocate.', background: 'Former Border Patrol agent.', education: [{ institution: 'University of Arizona', degree: 'Criminal Justice', year: 1998 }], experience: [{ title: 'Sector Chief', organization: 'Border Patrol', startYear: 1998, endYear: 2018, description: 'Led 2,000 agents' }], achievements: ['Record drug seizures', 'Anti-trafficking task force'] },
    ...
    },
    {
      ...
      bio: { summary: 'Fiscal conservative demanding balanced budgets.', background: 'Accountant and budget watchdog.', education: [{ institution: 'BYU', degree: 'Accounting', year: 1992 }], experience: [{ title: 'Director', organization: 'Citizens Against Government Waste', startYear: 2005, description: 'Identified $500B in waste' }], achievements: ['Balanced budget amendment advocate', 'State budget reform'] },
    ...
    },
    {
      ...
      bio: { summary: 'Foreign policy hawk committed to American strength.', background: 'Defense policy analyst.', education: [{ institution: 'Georgetown', degree: 'International Relations', year: 1996 }], experience: [{ title: 'Senior Fellow', organization: 'Heritage Foundation', startYear: 2008, description: 'Defense and foreign policy analysis' }], achievements: ['Published national security strategy', 'Pentagon advisory board'] },
    ...
    },
    {
      ...
      bio: { summary: 'Urban planner focused on affordable housing.', background: 'Transformed blighted neighborhoods.', education: [{ institution: 'Columbia', degree: 'Urban Planning', year: 2008 }], experience: [{ title: 'Planning Director', organization: 'City Planning Department', startYear: 2015, description: 'Led comprehensive plan update' }], achievements: ['Built 5,000 affordable units', 'Transit-oriented development'] },
    ...
    },
    {
      ...
      bio: { summary: 'Libertarian-leaning entrepreneur advocating for freedom.', background: 'Built multiple companies, tech investor.', education: [{ institution: 'Carnegie Mellon', degree: 'Computer Science', year: 2002 }], experience: [{ title: 'Investor', organization: 'Freedom Ventures', startYear: 2010, description: 'Backed 50 startups' }], achievements: ['Created 2,000 jobs', 'Criminal justice reform advocate'] },
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const avgSpectrum = candidate.topIssues.reduce((sum: number, i: any) => sum + i.spectrumPosition, 0) / candidate.topIssues.length;
    let leaning: 'progressive' | 'moderate_progressive' | 'centrist' | 'moderate_conservative' | 'conservative';
    if (avgSpectrum <= -70) leaning = 'progressive';
    else if (avgSpectrum <= -30) leaning = 'moderate_progressive';
    else if (avgSpectrum <= 30) leaning = 'centrist';
    ...
    const priorityIssues = candidate.topIssues.map((i: any) => i.issueId);

    const customPositions: Record<string, { position: string; spectrum: number }> = {};
    candidate.topIssues.forEach((i: any) => {
      customPositions[i.issueId] = { position: i.position, spectrum: i.spectrumPosition };
    });
    ...
    const candidateRef = doc(collection(db, Collections.CANDIDATES));
...
export const hasUserEndorsedCandidate = async (
  ...
  try {
    ...
    return snapshot.docs.some((d) => {
      const endorsement = d.data() as Endorsement;
      return (
...
export const ensureQuestionsExist = async (): Promise<void> => {
  try {
    const q = query(collection(db, Collections.QUESTIONS), firestoreLimit(1));
    const snapshot = await getDocs(q);
    if (snapshot.empty) {
...
export const reseedAllData = async (): Promise<void> => {
  ...
  issuesSnapshot.docs.forEach((d) => batch1.delete(d.ref));
  await batch1.commit();
  ...
  questionsSnapshot.docs.forEach((d) => batch2.delete(d.ref));
  await batch2.commit();
  ...
  candidatesSnapshot.docs.forEach((d) => batch3.delete(d.ref));
  await batch3.commit();
...
export const getPartyConfig = async (): Promise<PartyConfig | null> => {
  try {
    const q = query(collection(db, Collections.PARTY_CONFIG), firestoreLimit(1));
    const snapshot = await getDocs(q);
...
export const getProfileMetrics = async (
  ...
  const q = query(
    collection(db, Collections.PROFILE_METRICS),
    where('candidateId', '==', candidateId),
    where('date', '>=', startDateStr),
    orderBy('date', 'asc')
  );
  const snapshot = await getDocs(q);
  return snapshot.docs.map((d) => d.data() as ProfileMetrics);
};
...
export const getContestRounds = async (): Promise<ContestRound[]> => {
  try {
    const q = query(
      collection(db, Collections.CONTEST_ROUNDS),
      orderBy('order')
    );
    const snapshot = await getDocs(q);
    return snapshot.docs.map((d) => d.data() as ContestRound);
  } catch (error) {
...
export const seedContestRounds = async (): Promise<void> => {
  ...
  for (const round of CONTEST_ROUNDS_SEED) {
    const ref = doc(db, Collections.CONTEST_ROUNDS, round.id);
