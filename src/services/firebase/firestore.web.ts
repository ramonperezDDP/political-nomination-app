...
// PLAN-10E: Removed position templates and generation code.
...
export const seedCandidates = async (): Promise<void> => {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const fullTopIssues = candidate.topIssues.map((ti: any, idx: number) => ({
      ...ti,
      priority: idx + 1,
    }));
...
export const fixCandidateQuizResponses = async (): Promise<number> => {
  ...
  for (const candidateDoc of candidatesSnap.docs) {
    ...
    const userRef = doc(db, Collections.USERS, candidate.userId);
